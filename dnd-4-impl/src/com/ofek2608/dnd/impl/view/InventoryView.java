...
public final class InventoryView extends AbstractPlayerView {
	...
	public void buildEmbed(Context context, EmbedBuilder builder) {
		...
		if (data.detailed) {
			...
			builder.appendDescription(context.t(item.path) + ": " + count);
			if (backpackCount > 0)
				builder.appendDescription(" [(+" + backpackCount + ")](https://www.google.com)");
			builder.appendDescription("\n");

			builder.appendDescription(context.t(item.path + ".description"));
		} else {
	...
	private static class PlayerStats {
		...
		public PlayerStats(Player player) {
			Item[] items = player
					...
					.stream()
					.sorted()
					.map(com.ofek2608.dnd.data.Data::get)
